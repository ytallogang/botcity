...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oiee @${num.split('@')[0]}\ ❀⃘࣭࣭࣭࣭ٜꔷ⃔໑࣭࣭ٜ⟅Seja🔥Bem Vindos
🌴🐃 G∆D❍̸S & 🐄💕G∆D∆S a *${mdata.subject}* 
🌙𝗦𝗘𝗝𝗔 𝗔𝗧𝗜𝗩𝗢 𝗣𝗥𝗔 𝗡𝗔̃𝗢 𝗧𝗢𝗠𝗔𝗥 𝗕𝗔𝗡🎯`
				...
				teks = ` @${num.split('@')[0]}  𝗦𝗔𝗜𝗨 𝗦𝗘𝗠 𝗣𝗔𝗚𝗔𝗥 𝗔 𝗗𝗥𝗢𝗚𝗔 𝗙𝗗𝗣
 🤡🤏🏻🍁`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
