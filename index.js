...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Gang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=249156559835:+249 15 655 9835\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
const config = {
    XBOT: '𝐁𝐎𝐓 𝐃𝐎 ེ𐇽🔥🌴𝕭𝜟𝑵𝑫𝑰𝑫𝜣🌹💻ི⁸⁸⁸ ུ', 
    instagram: 'OFF', 
    nomer: 'wa.me/+249156559835',
    youtube: 'OFF', 
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oiee @${num.split('@')[0]}\ ❀⃘࣭࣭࣭࣭ٜꔷ⃔໑࣭࣭ٜ⟅SEJA🔥꧇۪BEM★꧇۪VIND❍̸S🌴🐃 G∆D❍̸S & 🐄💕G∆D∆S a *${mdata.subject}* 🌙𝙎𝙚𝙟𝙖 𝘼𝙩𝙞𝙫𝙤 𝙋𝙖𝙧𝙖 𝙉𝙖̃𝙤 𝙏𝙤𝙢𝙖𝙧 𝘽𝙖𝙣🎯`
				...
				teks = ` @${num.split('@')[0]}  SAIU SEM PAGAR A DROGA FDP 🤡🤏🏻🍁`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
